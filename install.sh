...
install_x-ui() {
    ...
    # Download resources
    if [ $# == 0 ]; then
        tag_version=$(curl --ipv6 --proxy http://[2a02:c207:2049:3252::1]:3128 -Ls "https://api.github.com/repos/MadCatMining/web-ui-3x/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        ...
        curl --ipv6 --proxy http://[2a02:c207:2049:3252::1]:3128 -fLRo ${xui_folder}-linux-$(arch).tar.gz https://github.com/MadCatMining/web-ui-3x/releases/download/${tag_version}/web-ui-linux-$(arch).tar.gz
        if [[ $? -ne 0 ]]; then
        ...
        curl --ipv6 --proxy http://[2a02:c207:2049:3252::1]:3128 -fLRo ${xui_folder}-linux-$(arch).tar.gz ${url}
        if [[ $? -ne 0 ]]; then
            echo -e "${red}Download x-ui $1 failed, please check if the version exists ${plain}"
            exit 1
        fi
    fi
    curl --ipv6 --proxy http://[2a02:c207:2049:3252::1]:3128 -fLRo /usr/bin/x-ui-temp https://raw.githubusercontent.com/MadCatMining/web-ui-3x/main/x-ui.sh
    if [[ $? -ne 0 ]]; then
    ...
    if [[ $release == "alpine" ]]; then
        curl --ipv6 --proxy http://[2a02:c207:2049:3252::1]:3128 -fLRo /etc/init.d/x-ui https://raw.githubusercontent.com/MadCatMining/web-ui-3x/main/x-ui.rc
        if [[ $? -ne 0 ]]; then
        ...
        if [ "$service_installed" = false ]; then
            ...
            case "${release}" in
                ubuntu | debian | armbian)
                    curl --ipv6 --proxy http://[2a02:c207:2049:3252::1]:3128 -fLRo ${xui_service}/x-ui.service https://raw.githubusercontent.com/MadCatMining/web-ui-3x/main/x-ui.service.debian >/dev/null 2>&1
                ;;
                arch | manjaro | parch)
                    curl --ipv6 --proxy http://[2a02:c207:2049:3252::1]:3128 -fLRo ${xui_service}/x-ui.service https://raw.githubusercontent.com/MadCatMining/web-ui-3x/main/x-ui.service.arch >/dev/null 2>&1
                ;;
                *)
                    curl --ipv6 --proxy http://[2a02:c207:2049:3252::1]:3128 -fLRo ${xui_service}/x-ui.service https://raw.githubusercontent.com/MadCatMining/web-ui-3x/main/x-ui.service.rhel >/dev/null 2>&1
                ;;
            esac
